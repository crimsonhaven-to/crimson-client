...
// Configuration: Adjust your backend URL domain here
...
function App() {
  ...
  // Triggered when a user performs a search lookup on landing page
  const handleSearchSubmit = (e) => {
    ...
    setSelectedTmdbId(searchId);

    // Endpoint 1: Fetch metadata overview mapping via TMDB ID
    fetch(`${API_BASE_URL}/info/${searchId}`)
      .then((res) => {
  ...
  // Triggered when an episode changes or a new anime loads
  useEffect(() => {
    ...
    // Endpoint 2: Primary Streaming Links Orchestrator via AniList ID
  ...
  return (
    <div className="min-h-screen bg-crimson-950 text-crimson-100 font-sans selection:bg-crimson-500 selection:text-white flex flex-col justify-between relative overflow-x-hidden">
      ...
      <nav className="border-b border-crimson-900/60 bg-crimson-950/80 backdrop-blur-md sticky top-0 z-50 px-6 py-4 flex items-center justify-between shadow-md">
        <div 
          onClick={() => { setCurrentView('landing'); setApiError(null); }} 
        ...
        <div className="flex space-x-6 text-sm font-medium items-center">
          <button 
            ...
            className={`transition-colors flex items-center gap-1.5 ${currentView === 'landing' ? 'text-crimson-500 font-bold' : 'text-crimson-200/70 hover:text-crimson-400'}`}
          >
            <Film className="w-4 h-4" /> Search Home
          </button>
          <button 
            onClick={() => setCurrentView('about')} 
            className={`transition-colors flex items-center gap-1.5 ${currentView === 'about' ? 'text-crimson-500 font-bold' : 'text-crimson-200/70 hover:text-crimson-400'}`}
          >
        ...
        {currentView === 'landing' && (
          <div className="max-w-3xl w-full mx-auto px-6 py-20 text-center space-y-8 my-auto">
            ...
            <form onSubmit={handleSearchSubmit} className="relative max-w-xl mx-auto group">
              <input 
                type="number"
                placeholder="Enter TMDB Identification Key... (e.g., 127230)"
                value={searchId}
                onChange={(e) => setSearchId(e.target.value)}
                className="w-full bg-crimson-900/20 border-2 border-crimson-900/80 text-white rounded-2xl py-4 pl-5 pr-14 focus:outline-none focus:border-crimson-500 focus:ring-4 focus:ring-crimson-500/10 placeholder-crimson-700/80 font-medium tracking-wide shadow-2xl transition-all appearance-none"
              />
              <button 
                type="submit" 
                disabled={metaLoading}
                className="absolute right-2 top-2 bottom-2 bg-crimson-500 hover:bg-crimson-400 disabled:bg-crimson-800 text-white px-4 rounded-xl transition-all shadow-md flex items-center justify-center"
              >
                {metaLoading ? (
            ...
            {apiError && (
              <div className="max-w-md mx-auto p-4 bg-crimson-900/40 border border-crimson-500/30 rounded-xl text-sm text-crimson-300 flex items-center gap-3 shadow-lg">
                  ...
                  <React.Fragment>
                    {streamData.streams[activeStreamIdx].type === 'iframe' ? (
                      <iframe
                        ...
                        title="Stream Iframe fallback"
                        className="w-full h-full"
                        allowFullScreen
                        scrolling="no"
                      />
                    ) : (
                      /* Handle Native playback streams (MP4/HLS) safely via generic player message */
                ...
                <div className="flex flex-wrap items-start justify-between gap-4 relative z-10">
                  <div className="space-y-2 max-w-xl">
                    <div className="flex gap-2 items-center">
                      ...
                      <span className="text-xs text-crimson-400/80 font-mono">AniList: {selectedAnilistId}</span>
                    </div>
                    <h1 className="text-3xl font-black tracking-tight text-white">
                      {animeMetadata?.title || "Unknown Cluster Title"}
                    </h1>
                    <p className="text-sm text-crimson-200/70 leading-relaxed text-justify line-clamp-3 hover:line-clamp-none transition-all cursor-pointer">
                      {animeMetadata?.summary || "No description indexes provided by external TMDB datasets."}
                    </p>
                  </div>
        ...
        {currentView === 'about' && (
          <div className="max-w-2xl w-full mx-auto px-6 py-12 space-y-6 my-auto">
            ...
            <div className="space-y-4 text-sm text-crimson-200/80 leading-relaxed text-justify">
              <p>
                <strong>crimsonhaven</strong> is a performance-optimized high-fidelity user application frame. 
                It functions explicitly as an abstraction layer built on top of decoupled microservices designed to format 
                metadata processing endpoints.
              </p>
              <p>
                By coupling an asynchronous pipeline backend with an intuitive modern client interface styled using real-time variable configurations, 
                this platform allows parsing clean media packages instantly without tracking structural analytics or bloating system resources.
              </p>
