...
import { Search, Play, HelpCircle, Film, Info, AlertTriangle, ChevronRight, Server, RefreshCw, Layers } from 'lucide-react';
import Background from './assets/background.jpg';
import { useAnimeSearch, useTrendingAnime, useRouter } from './hooks';
...
const AnimeCard = ({ title, poster, onSelect }) => (
    <div className="flex items-center justify-between p-3 cursor-pointer hover:bg-crimson-900/20 transition-colors border-b border-crimson-900/50" onMouseDown={onSelect}>
        <div className="flex items-center gap-3">
            {poster ? <img src={poster} alt="" className="w-12 h-auto object-cover rounded shadow-lg flex-shrink-0" /> : <div className="w-12 h-16 bg-crimson-900/30 flex items-center justify-center text-sm text-crimson-400">No Poster</div>}
...
function LandingView({ searchState, trendingAnimes, trendLoading, onSelectSuggestion }) {
    const { queryName, setQueryName, searchResults, showSuggestions, setShowSuggestions, metaLoading, apiError } = searchState;
    const dropdownRef = useRef(null);
    const handleSearchSubmit = (e) => { e.preventDefault(); if (!queryName.trim()) return; if (searchResults.length > 0) onSelectSuggestion(searchResults[0]); };
    return (
        <div className="max-w-3xl w-full mx-auto px-6 py-20 text-center space-y-8 my-auto">
            <div className="space-y-3">
                <h1 className="text-6xl font-black tracking-tight text-white uppercase drop-shadow-[0_4px_12px_rgba(255,0,60,0.15)]">crimson<span className="text-crimson-500 font-light">haven</span></h1>
                <p className="text-crimson-300 text-lg tracking-wide font-medium">Stream dynamic links seamlessly straight from the dark network.</p>
            </div>
            <div className="relative max-w-xl mx-auto" ref={dropdownRef}>
                <form onSubmit={handleSearchSubmit} className="flex items-end space-x-2 border-2 border-crimson-900/80 rounded-2xl shadow-2xl bg-crimson-900/30 transition-all">
                    <input type="text" placeholder="Enter Anime Name (e.g., The Eminence in Shadow)..." value={queryName} onFocus={() => queryName.length >= 3 && setShowSuggestions(true)} onChange={(e) => { const val = e.target.value; setQueryName(val); if (val.trim().length >= 3) setShowSuggestions(true); }} onBlur={(e) => { if (!dropdownRef.current?.contains(e.relatedTarget)) setShowSuggestions(false); }} className="w-full text-white py-4 px-5 focus:outline-none placeholder-crimson-400/90 placeholder:drop-shadow-[0_0_6px_rgba(239,68,68,0.5)] font-semibold tracking-wide appearance-none bg-transparent" />
                    <button type="submit" disabled={metaLoading} className="bg-crimson-500 hover:bg-crimson-400 disabled:bg-crimson-800 text-white px-6 py-[18px] rounded-r-2xl transition-all shadow-md flex items-center justify-center self-stretch">{metaLoading ? <div className="w-5 h-5 border-2 border-white border-t-transparent rounded-full animate-spin" /> : <Search className="w-5 h-5" />}</button>
                </form>
                {showSuggestions && (
                    <div className="absolute top-full left-0 right-0 mt-1 bg-crimson-950 border border-crimson-800 shadow-xl max-h-[300px] overflow-y-auto z-20 text-left rounded-xl">
                        {searchResults.length > 0 ? searchResults.map((suggestion, index) => <AnimeCard key={index} title={suggestion.title || suggestion.name} poster={suggestion.poster || null} onSelect={() => onSelectSuggestion(suggestion)} />) : <div className="p-4 text-sm text-crimson-400 text-center italic">No tracked anime found matching that title.</div>}
                    </div>
                )}
            </div>
            {apiError && <div className="max-w-md mx-auto p-4 bg-crimson-900/40 border border-crimson-500/30 rounded-xl text-sm text-crimson-300 flex items-center gap-3 shadow-lg mt-8"><AlertTriangle className="w-5 h-5 text-crimson-500 shrink-0" /><span className="text-left">Status Message: {apiError}</span></div>}
            <div className="mt-16 pt-8 border-t border-crimson-900/50">
                <h2 className="text-3xl font-black tracking-tight text-white uppercase mb-6 flex items-center gap-2"><Play className="w-6 h-6 text-crimson-500" /> Trending Streams <span className="text-base font-normal opacity-70">({trendingAnimes.length} Results)</span></h2>
                {trendLoading ? <div className="flex justify-center space-x-6 animate-pulse text-crimson-300">{[1, 2, 3].map((n) => <div key={n} className="w-[150px] h-48 bg-gray-700/30 rounded-lg border border-dashed border-crimson-900/50" />)}</div> : trendingAnimes.length > 0 ? <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-6">{trendingAnimes.map((anime, index) => <div key={index} onClick={() => onSelectSuggestion(anime)} className="bg-crimson-900/10 border border-crimson-900/40 rounded-xl overflow-hidden hover:border-crimson-500 transition-all group cursor-pointer transform hover:-translate-y-1"><img src={anime.poster} alt={`${anime.title} poster`} className="w-full h-auto object-cover" /><div className="p-3 text-left"><h4 className="text-sm font-bold text-white line-clamp-2 group-hover:text-crimson-400 transition-colors">{anime.title}</h4></div></div>)}</div> : <div className="p-6 bg-crimson-900/20 rounded-xl border border-dashed border-crimson-900 text-center text-crimson-500">No currently tracked popular streams found.</div>}
            </div>
        </div>
    );
}

// --- FIXED MULTI-SEASON DISTINCTION WATCH VIEW ---
function WatchView({ routeParams, navigate, searchState }) {
    const { anilistId, season, episode } = routeParams;
    const { animeMetadata, setAnimeMetadata } = searchState;
    const [activeStreamIdx, setActiveStreamIdx] = useState(0);
    const [streamData, setStreamData] = useState(null);
    const [streamLoading, setStreamLoading] = useState(false);

    // Sync metadata parameters if accessed direct via explicit system URL links
    useEffect(() => {
        if (!anilistId) return;
        ...
        fetch(`${API_BASE_URL}/info/${anilistId}`)
            ...
            .catch(err => console.error("Metadata sync err:", err));
    }, [anilistId, animeMetadata, setAnimeMetadata]);

    // Fetch stream resources explicitly binding BOTH season and episode parameters
    useEffect(() => {
        if (!anilistId) return;
        ...
        // FIXED: Explicit pass mapping binding both target fields to backend routers
        fetch(`${API_BASE_URL}/watch/${anilistId}/${season}/${episode}`)
            .then((res) => {
    ...
    // Extract out distinct array mapping vectors for explicit multi-season lists
    const distinctSeasons = animeMetadata?.episodes_list
        ? [...new Set(animeMetadata.episodes_list.map(ep => ep.season_number || 1))].sort((a,b) => a - b)
        : [1];

    // Filter episodes to only render items matching our active URL context path tracking state
    const filteredEpisodes = animeMetadata?.episodes_list
        ? animeMetadata.episodes_list.filter(ep => (ep.season_number || 1) === Number(season))
        : [];
        ...
        <div className="max-w-7xl w-full mx-auto px-4 py-8 grid grid-cols-1 lg:grid-cols-4 gap-8">
                ...
                <div className="p-6 bg-crimson-900/10 border border-crimson-900/40 rounded-2xl backdrop-blur-sm">
                    <div className="flex flex-wrap items-start justify-between gap-4">
                        <div className="space-y-2 max-w-xl">
                            ...
                            <p className="text-sm text-crimson-200/70 leading-relaxed text-justify line-clamp-3 hover:line-clamp-none transition-all cursor-pointer">{animeMetadata?.summary || 'No description indexes provided.'}</p>
                        </div>
                        <div className="flex gap-2">
                            <div className="bg-crimson-950/60 border border-crimson-900/60 px-4 py-3 rounded-xl text-center min-w-[80px]">
                                <p className="text-[10px] uppercase text-crimson-500 font-extrabold tracking-widest">Season</p>
                                <p className="text-2xl font-black text-white">{season}</p>
                            </div>
                            <div className="bg-crimson-900/40 border border-crimson-800/40 px-4 py-3 rounded-xl text-center min-w-[80px]">
                                <p className="text-[10px] uppercase text-crimson-400 font-extrabold tracking-widest">Episode</p>
                                <p className="text-2xl font-black text-white">{episode}</p>
                            </div>
                        </div>
                    </div>
                </div>

                {/* FIXED: Dynamic Two-Tier Cluster Rendering Interface for Multi-Season Parameters */}
                {animeMetadata?.episodes_list && (
                    <div className="space-y-4">
                        {/* Tier 1: Render Season Switcher Tabs if multiple seasons found */}
                        {distinctSeasons.length > 1 && (
                            <div className="p-4 bg-crimson-950/40 border border-crimson-900/40 rounded-2xl flex items-center gap-3 overflow-x-auto">
                                <span className="text-xs font-bold uppercase text-crimson-500 flex items-center gap-1 shrink-0"><Layers className="w-3.5 h-3.5"/> Seasons:</span>
                                <div className="flex gap-2">
                                    {distinctSeasons.map(sNum => (
                                        <button
                                            key={sNum}
                                            onClick={() => navigate(`/watch/${anilistId}/${sNum}/1`)}
                                            className={`px-4 py-1.5 rounded-lg text-xs font-bold transition-all border ${
                                                Number(season) === sNum 
                                                    ? 'bg-crimson-500 border-crimson-400 text-white' 
                                                    : 'bg-crimson-900/10 border-crimson-900/50 text-crimson-300 hover:border-crimson-700'
                                            }`}
                                        >
                                            Season {sNum}
                                        </button>
                        ...
                        {/* Tier 2: Render targeted filtered episode grid matching exact season parameter */}
                        <div className="p-6 bg-crimson-900/10 border border-crimson-900/40 rounded-2xl space-y-4">
                            ...
                            <div className="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-6 gap-2">
                                {filteredEpisodes.map((ep, idx) => (
                                    <button 
                                        key={idx} 
                                        onClick={() => navigate(`/watch/${anilistId}/${season}/${ep.episode_number}`)} 
            ...
            <div className="lg:col-span-1 space-y-4">
                <div className="bg-crimson-900/20 border border-crimson-900/50 p-6 rounded-2xl sticky top-24">
                    <h3 className="text-lg font-bold text-white mb-4 flex items-center gap-2"><span className="w-2 h-2 rounded-full bg-crimson-500 animate-ping" /> Scraped Targets</h3>
                    <div className="space-y-2">
                        {streamLoading ? [1, 2].map((n) => <div key={n} className="h-14 bg-crimson-900/10 animate-pulse rounded-xl border border-crimson-900/20" />) : streamData?.streams && streamData.streams.length > 0 ? streamData.streams.map((stream, idx) => (
...
function AppContent() {
    ...
    const handleSelectSuggestion = async (suggestion) => {
        ...
        try {
            ...
            // FIXED: Initial click from landing or trending defaults paths to Season 1 / Episode 1
            navigate(`/watch/${data.anilist_id}/1/1`);
        } catch (err) {
    ...
    return (
        <div className="min-h-screen bg-crimson-950 text-crimson-100 font-sans selection:bg-crimson-500 selection:text-white flex flex-col justify-between relative overflow-x-hidden">
            <div className="absolute inset-0 pointer-events-none z-0"><img src={Background} alt="background wallpaper" className="w-full h-full object-cover opacity-50 wallpaper-img" /></div>
            <nav className="border-b border-crimson-900/60 bg-crimson-950/80 backdrop-blur-md sticky top-0 z-50 px-6 py-4 flex items-center justify-between shadow-md">
                <div onClick={() => { navigate('/'); searchState.setApiError(null); }} className="flex items-center space-x-2 cursor-pointer group"><span className="text-2xl font-black tracking-tighter text-crimson-500 group-hover:text-crimson-400 transition-colors">crimson<span className="text-crimson-100 font-light">haven</span></span></div>
                <div className="flex space-x-6 text-sm font-medium items-center">
                    <button onClick={() => navigate('/')} className={`transition-colors flex items-center gap-1.5 ${view === 'landing' ? 'text-crimson-500 font-bold' : 'text-crimson-200/70 hover:text-crimson-400'}`}><Film className="w-4 h-4" /> Search Home</button>
                    <button onClick={() => navigate('/about')} className={`transition-colors flex items-center gap-1.5 ${view === 'about' ? 'text-crimson-500 font-bold' : 'text-crimson-200/70 hover:text-crimson-400'}`}><HelpCircle className="w-4 h-4" /> About Us</button>
                </div>
            </nav>
            <div className="flex-grow z-10 flex flex-col justify-center">
                {view === 'landing' && <LandingView searchState={searchState} trendingAnimes={trendingAnimes} trendLoading={trendLoading} onSelectSuggestion={handleSelectSuggestion} />}
                {view === 'watch' && <WatchView routeParams={params} navigate={navigate} searchState={searchState} />}
                {view === 'about' && <AboutView />}
            </div>
            <footer className="w-full border-t border-crimson-900/40 bg-crimson-950/90 text-center py-6 px-4 z-10 relative">
                <p className="text-[11px] font-medium tracking-wide text-crimson-600 max-w-3xl mx-auto uppercase leading-normal">Disclaimer: <span className="text-crimson-400/70">crimsonhaven does not host any assets locally.</span></p>
            </footer>
        </div>
    );
}

function App() { return ( <GlobalErrorBoundary> <AppContent /> </GlobalErrorBoundary> ); }
export default App;