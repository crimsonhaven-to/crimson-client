...
const LoginWall = () => {
  ...
  // --- sign in / register (shared shell with a tab switch) ------------------
  const isRegister = mode === 'register';
  return (
    <Shell subtitle="Members only — sign in to descend">
      {/* Tab switch */}
      <div className="grid grid-cols-2 gap-2 p-1.5 bg-crimson-950/40 border border-crimson-900/60 rounded-2xl mb-8">
        {[['login', 'Sign In'], ['register', 'Register']].map(([key, label]) => (
          <button key={key} onClick={() => switchMode(key)}
            className={`py-2.5 rounded-xl text-[11px] font-black uppercase tracking-[0.2em] transition-all ${
